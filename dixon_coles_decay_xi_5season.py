...
if __name__ == '__main__':
    ...
    dataset = pd.read_csv("E0.csv")
    #dataset['Date'] = pd.to_datetime(dataset['Date'])
    dataset['Date'] = pd.to_datetime(dataset['Date'],  format='%d/%m/%Y')
    dataset['time_diff'] = (max(dataset['Date']) - dataset['Date']).dt.days
    dataset = dataset[['HomeTeam', 'AwayTeam', 'FTHG', 'FTAG', 'FTR', 'time_diff']]
    dataset = dataset.dropna(how='all')
    # value of xi you wish to test
    my_xi = 0.00325
    # skip days where no matches took place
    match_days = [day for day in range(99,-1,-3) if len(dataset[((dataset['time_diff']<=day) & (dataset['time_diff']>=(day-2)))])]
    print("Pooling...")
